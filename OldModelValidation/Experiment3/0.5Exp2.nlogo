...
turtles-own [reserves c_rate m_rate]      ;Current resources, Consumption rate, and motility rate
globals [
...
This model demonstrates the relationship between metabolic rate and optimal motility in competing cell populations. 
...
2. Consume resources from the local microenvironment according to metabolic rate

3. If energy is above reproduction threshold, reproduce. Cells split their energy between two daughter cells upon reproduction. Motility rate mutates in 1 daughter cell every time a cell reproduces, and the size of the mutation is a random number drawn from a distribution with a mean 0 and a SD of 0.01. Daughter cells are placed in the same microenvironment as parent.

4. Die if microenvironment resources or internal reserves are below death threshold;

Then,

For each microenvironment, renew resource according to rate of renewal.
...
> death_threshold - The level of resources in a microenvironment required to trigger death of all resident cells.
