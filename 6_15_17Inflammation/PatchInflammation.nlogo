...
to go-patches
 ...
 ;;; FOR THIS EXPERIMENT ONLY - ADD INFLAMMAGEN TO LEFT SIDE OF WORLD
 if pxcor < 0
 [
...
"" ""
PENS
"Left Normal" 1.0 0 -7171555 true "" "plot (count turtles with [xcor < 0 and color = yellow])/(count turtles with [xcor < 0])"
"Right Normal" 1.0 0 -723837 true "" "plot (count turtles with [xcor >= 0 and color = yellow])/(count turtles with [xcor >= 0])"
"Left Neoplastic" 1.0 0 -14730904 true "" "plot (count turtles with [xcor < 0 and color = blue])/(count turtles with [xcor < 0])"
"Right Neoplastic" 1.0 0 -8020277 true "" "plot (count turtles with [xcor >= 0 and color = blue])/(count turtles with [xcor >= 0])"
...
"" ""
PENS
"Normal" 1.0 0 -1184463 true "" "plot (count turtles with [xcor >= 0 and color = yellow])/(count turtles with [xcor >= 0])"
"Cancer" 1.0 0 -13345367 true "" "plot (count turtles with [xcor >= 0 and color = blue])/(count turtles with [xcor >= 0])"
...
This model demonstrates the relationship between metabolic rate, motility, neoplasticity, and inflammation, in competing cell populations. Inflammation here is solely external, arbitrarily occuring only on the left side of the world.
...
> patch_inflammation - the amount of inflammagen added per time step to the patches on the left side of the world.
...
The growth of one population can be simulated by setting mutation rate to zero. The effect of ambient inflammation, equivalent to resource disparity, on neoplastic dominance and motility, is the key question.
