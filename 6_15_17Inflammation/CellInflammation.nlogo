...
turtles-own [reserves c_rate m_rate maint_cost inflammogenicity]      ;Current resources, Consumption rate, motility rate, ability to create inflammagens
globals [
...
to setup-cells
 ...
 set inflammogenicity 0                         ; Set initial inflammogenicity to 0
 set shape "circle"
...
to go-cells
 ...
 eat
 set inflammagen (inflammagen + inflammogenicity)    ; Add inflammagen to microenvironment
...
to reproduce
 set reserves reserves / 2     ; Halve reserves, to be distributed amongst daughter cells
  ...
  if c_evo? = true     ; Mutate consumption rate
   [
   set c_rate c_rate + (random-normal 0 mut_sd) ;mutates c_rate to M=0, SD=mut_sd
   ]

  if m_evo? = true     ; Mutate mobility rate
   [
   set m_rate m_rate + (random-normal 0 mut_sd) ;mutates m_rate to M=0, SD=mut_sd
  ...
  if infl_evo? = true   ; Mutate inflammogenicity
  [
    set inflammogenicity (inflammogenicity + (random-normal 0 mut_sd))  ;mutates inflammogenicity to M=0, SD= mut_sd

    if inflammogenicity < 0                     ;Sanity check inflammogenicity, can't be negative.
    [set inflammogenicity 0]
  ]
...
4. Add inflammagen to the microenvironment based on cellular inflammogenicity.

5. If energy is above reproduction threshold, reproduce. Cells split their energy between two daughter cells upon reproduction. Motility rate mutates in 1 daughter cell every time a cell reproduces, and the size of the mutation is a random number drawn from a distribution with a mean 0 and a SD of 0.01. Daughter cells are placed in the same microenvironment as parent.

6. Die if internal reserves are below zero;
...
> infl_evo? - Toggles the evolution of inflammogenicity
