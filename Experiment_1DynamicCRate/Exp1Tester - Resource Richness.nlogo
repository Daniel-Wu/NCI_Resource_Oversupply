;;init var
patches-own [energy competitiveness relative_richness]      ;Resources in patch, inflammagenic compounds in patch
...
to setup-tissue
 ask patches
  [
  sprout number_cells ;(floor renewal_rate / base_maint_cost)     ; Initializes with exactly a carrying capacity of cells.
   [
...
to distribute-energy
  ...
  let total_demand (sum [c_rate] of turtles-here)

  ifelse total_demand < energy
  [
  ...
  [
    ...
    ask turtles-here [
      set reserves (reserves + (c_rate / total_demand) * energy)
    ]
...
;Runs a relevant test - here resource richness
to test
  ask patches [
    set competitiveness (sum [c_rate] of turtles-here)

  ]

  ask patches [
    set relative_richness ((mean [competitiveness] of neighbors) - competitiveness)
...
687
1032
Relative Richness of Mutant Patches
Time
Relative Richness
0.0
10.0
...
"" ""
PENS
"Mutant" 1.0 0 -16777216 true "" "plot mean [relative_richness] of (turtles with [mutated? = true])"
"Average" 1.0 0 -7500403 true "" "plot mean [relative_richness] of patches"
"Cancer" 1.0 0 -13345367 true "" "plot mean [relative_richness] of (turtles with [c_rate > 1 and m_rate > 0])"
...
1074
1034
Average Competitiveness in Microenvironments
Time
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot mean [competitiveness] of patches"
...
The simulation is terminated when the end-time is reached, or when neoplastic cells reach fixation, defined as 90% of the population. A cell is designated neoplastic when its motility and consumption are both higher than that of normal cells.
